#!/usr/bin/env node
/** Apply migration 029 OpenPay billing tables. */
...
const MIGRATION = '029_openpay_billing.sql';
...
for (let i = 0; i < candidates(databaseUrl).length; i++) {
  ...
  process.stdout.write(`Applying 029 via ${label} … `);
  ...
  try {
    ...
    const tables = await client.query(`
      ...
      WHERE table_schema='public'
        AND table_name IN ('subscriptions','payments','invoices','payment_events','payment_logs')
      ORDER BY table_name
    `);
